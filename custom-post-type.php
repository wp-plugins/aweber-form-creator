...
*/
$mce_form_count = '';
class AweberFormCreatorCustomPostType{

private $post_type = 'aweberFormCreator';
private $post_label = 'Aweber Form Creator';
private $prefix = '_aweber_form_creator_';
function __construct() {
	
	add_filter( 'cmb_meta_boxes', array(&$this,'metaboxes' ));
	add_action( 'init', array(&$this,'initialize_meta_boxes'), 9999 );
	add_action("init", array(&$this,"create_post_type"));
	add_action( 'init', array(&$this, 'aweber_form_creator_register_shortcodes'));
...
function metaboxes( array $meta_boxes ) {
	...
	//$prefix = '_aweber_form_creator_';
	

	$meta_boxes[] = array(
		'id'         => 'aweber_form_metabox',
		'title'      => 'Aweber Form Creator',
		...
		'fields'     => array(
			...
			*/
			...
			array(
				'name' => 'Aweber List Name',
				'desc' => 'Enter the Aweber list name here.',
				'id'   => $this->prefix . 'aweber_listname',
				'type' => 'text',
				'std'  => '',
			),
...
function aweber_form_creator_shortcode($atts){
		...
		$aweber_listname = get_post_meta($id, $this->prefix . 'aweber_listname', true);
		
		ob_start();
		include $dir.'template/aweberFormCreatorTemplate.php';
		return ob_get_clean();
}



function aweber_form_creator_register_shortcodes(){
		add_shortcode( 'aweber_form_creator', array(&$this,'aweber_form_creator_shortcode' ));
	}
...
function enqueue_styles(){
	wp_register_style( 'aweber-form-creator-css', plugin_dir_url(__FILE__).'css/aweberFormCreator.css' );
	wp_enqueue_style('aweber-form-creator-css');
}

function enqueue_scripts(){
	wp_enqueue_script('aweber-form-creator-js', plugin_dir_url(__FILE__).'js/aweberFormCreator.js');
}
...
new AweberFormCreatorCustomPostType();
